...
;----------
; bjw_specpro
; Changes by Benjamin Weiner for robustness / working on Binospec data
; rename main function, so call it as bjw_specpro, 1, /basic  (etc)
;
; in spec_template_update: add check for usingbasicversion before
;   accessing sed_fit_flag
; in makeplot: ignore the reddest and bluest 5% of spectrum where
;   there is data to improve y scaling in 1-d spec
...
; in make2dplot: expand the zoom plot by 4x4 pixels to screen pixels,
...
pro spec_template_update, event, info
  ...
  case event.index of 
    0: begin
      ...
      specpro_set_state, event, info
      ;Update SED fit
      ; BJW - add check for usingbasicversion
      if info.usingbasicversion ne 1 then begin
...
pro like_redshift, event, info
  specpro_get_state,event,info
  ; BJW - change redshift format to F7.4, not F5.3; this is saved to file
  zstring = string(info.redshift, format = '(F7.4)') 
  ...
  dir = getenv('SPVIEW')
   ...
   if n_elements(minz) ne 0 then begin
      ...
      if zpdfup[0] gt zpdflow[0] and abs(zpdfup[0]) lt 7 and abs(zpdflow[0]) lt 7 then begin
        zmin = zpdflow[0]
        zmax = zpdfup[0]
;      endif else if zpdflow[0] lt 0.001 and zpdfup[0] lt 0.001 then begin
;      ; BJW - trap if the zpdf values are just 0, set to a normal zrange
...
;        zmax = zrange[1]
   ...
   endelse
    ...
    ; BJW - print z range for debugging
    print, "Entered zfindspec with zmin,zmax: ",zmin,zmax," results: ",zstring
...
; BJW - change region in which max and min flux are measured - 
; exclude the bluest and reddest 5% of spectrum. This will make a
; better display for spectra that have poor data and up/down turn at
; the ends.
minlam = min(alllambda)
maxlam = max(alllambda)
minplotlim = minlam + 0.05 * (maxlam-minlam)
maxplotlim = maxlam - 0.05 * (maxlam-minlam)
iplotlim = where(alllambda gt minplotlim and alllambda lt maxplotlim, icount)
if icount gt 0 then begin
   minflux = min(allspec(iplotlim))
   maxflux = max(allspec(iplotlim))
endif 
...
if info.usingsmallversion ne 1 then begin
  ...
  if zoom ne 0 and n_elements(zoomevent) ne 0 then begin
    ...
    endelse
     ...
     ;appropriately.
     ; BJW - debug errors from xzoomregion 
     xzoomregion = where(alllambda gt xmin and alllambda lt xmax, icount)
     if icount lt 1 then begin
        print, "makeplot: null xzoomregion - xmin, xmax: ",xmin, xmax
        ; this will fix indexing error but not further issues with xmin, xmax
        xzoomregion = iplotlim
     endif
    ...
    endelse
     ...
     ;appropriately.
     ; BJW - debug errors from xzoomregion 
     xzoomregion = where(alllambda*sc gt xmin and alllambda*sc lt xmax, icount)
     if icount lt 1 then begin
        print, "makeplot: null xzoomregion - xmin, xmax: ",xmin, xmax
        ; this will fix indexing error but not further issues with xmin, xmax
        xzoomregion = iplotlim
...
;      ovl=where(alllambda gt templmin and alllambda lt templmax and abs(allspec) lt 1e30 ,ovlcnt)
      ; BJW - try normalizing only within the plot window?
      ovl=where(alllambda gt templmin and alllambda lt templmax and abs(allspec) lt 1e30 and alllambda gt xmin and alllambda lt xmax,ovlcnt)
      ...
      if ovlcnt gt 0 then begin
        ...
        ; BJW - try a simpler unweighted normalization for template
        temp_int = interpol(tempspec, templambda*(1.0+z), alllambda(ovl))
        normfac = total(allspec(ovl)) / total(temp_int)

        tempnorm = tempspec*normfac ;normalize template
        tempzp = mean(allspec(ovl)) ;get spec mean
        ; BJW: debugging
        print,"makeplot: template normalization, zp: ",normfac,tempzp
...
pro make2Dplot, z, linetemplates, showspecpos, specpos, $
  ...
  ; range determination. This is not working well.
  ...
  if n_elements(zoombox) ne 0 then begin
     ...
     ; BJW - make sure these are integers
     ...
     ; BJW - this is the previous version of code before expansion
...
;-----------------------------------------------------------------------
pro bjw_specpro, slit, small=small, space=space, basic=basic, histogram=histogram, OH=OH
